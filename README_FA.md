...
## نسخه v5
این نسخه بدون حذف اطلاعات قبلی، قابلیت‌های زیر را اضافه می‌کند:
- بخش Inbox / ورودی‌ها در منوی سمت راست
- نمای روزانه شبیه Google Calendar از ۰۰:۰۰ تا ۲۳:۵۹
- تمرکز، ۳ هدف و مرور پایان هفته در بخش هفتگی
- نمودار عملکرد هفتگی، ماهیانه و سالیانه در داشبورد مدیریتی
- رنگ‌بندی اولویت‌ها در نمودار دایره‌ای: بالا قرمز، متوسط زرد، پایین آبی
- بخش اهداف سالانه، فصلی، ماهانه و حاشیه برتری
- تعریف گام برای هر هدف و محاسبه خودکار درصد پیشرفت
- Habit Tracker ماهانه با نقشه حرارتی سبز، چک‌لیست روزانه و هدف روزانه/چند روز در هفته

اطلاعات قبلی حذف نمی‌شود؛ داده‌ها همچنان در Supabase و LocalStorage ذخیره می‌شوند. این نسخه فقط ساختار JSON ذخیره‌شده را توسعه می‌دهد.


## تغییرات نسخه v5

- در نمای روزانه، فعالیت‌های دارای ساعت شروع و پایان اکنون کل بازه زمانی را مثل Google Calendar رنگی می‌کنند.
- نقشه حرارتی عادت‌ها داخل کادر خود فیت شده و از صفحه بیرون نمی‌زند.
- سایدبار و بخش همگام‌سازی ابری اسکرول داخلی دارد تا پنل حساب ابری مخفی نشود.
- کارت‌های اهداف فشرده و جمع‌شونده شده‌اند تا تعداد زیاد هدف صفحه را بیش از حد بلند نکند.
- فیلد «شاخص سنجش» از اهداف حذف شد؛ «چرایی»، «نتیجه مطلوب»، «مهلت» و گام‌ها باقی ماند.
- مهلت هدف با انتخابگر تاریخ ثبت می‌شود و در داده‌ها به تاریخ شمسی ۱۴۰۵ تبدیل می‌گردد.
- نوار پیشرفت هدف برجسته‌تر شد و بر اساس تکمیل گام‌ها به‌صورت خودکار پر می‌شود.


## تغییرات نسخه v6

- جلوگیری از گیر کردن فرم ورود روی «در حال ورود...».
- اضافه شدن Timeout برای ورود، دریافت و ذخیره Supabase.
- اضافه شدن دکمه «تست اتصال» داخل پنجره ورود ابری.
- اگر Supabase کند یا مسدود باشد، برنامه به‌جای ماندن روی Loading پیام واضح نشان می‌دهد و داده‌ها را لوکال حفظ می‌کند.
